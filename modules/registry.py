...
class RegistryExtractor(ArtifactExtractor):
    ...
    def __init__(self, output_dir, config):
        ...
        self.hive_csv_file = open(os.path.join(
            self.registry_output_dir, "registry_hive_files.csv"), "w+", newline="", encoding="utf-8")

        self.writer = csv.writer(self.hive_csv_file)
        self.writer.writerow(["Hive", "Hive Path", "Value Name", "Value Type",
                             "Value Data", "Created At", "Modified At", "Deleted At"])
    ...
    def process_fs_object(self, fs_object, file_path):
        try:
            ...
            if "RegBack".lower() in file_path.decode("utf-8").lower():
                ...
                outfile_path = os.path.join(
                    self.registry_output_dir, file_name.decode("utf-8"))
                self.extract_key_values(outfile_path, file_name)
        except IOError:
            pass
    ...
    def extract_key_values(self, file_path, file_name):
        ...
        with open(csv_file_path, "w+", newline="", encoding="utf-8") as csv_file:
            ...
            writer.writerow(["Hive Path", "Key Path", "Value Name",
                            "Value Type", "Value Data", "Created At"])
                ...
                if hive_name.lower() == file_name.decode("utf-8").lower():
                    try:
                        ...
                        if value_name:
                            ...
                            writer.writerow([file_name.decode(
    ...
    def get_file_path(self, fs_object):
        ...
        while current_object.info.name.name not in [b".", b".."]:
            file_path_parts.insert(
                0, current_object.info.name.name.decode("utf-8"))
    ...
    def write_registry_hive(self, fs_object, file_path, file_name, file_suffix):
        ...
        self.writer.writerow(
            [file_name.decode("utf-8"), file_path, "", "", "", create, change, modify, ""])
